...
/**
 * 用于匹配karma.conf.js中file列表的配置项
 * @param {String} pattern glob pattern格式的文件匹配符
 * @param {Boolean} served 是否由服务端提供请求
 * @param {Boolean} included 是否在模板内通过script.src引入
 * @param {Boolean} watched 是否监听文件变化
 * @constructor
 */
...
var normalizeConfig = function(config, configFilePath) {
    // 根据config.basePath计算相对路径relativePath
    // 返回绝对路径
    ...
    /**
     * 对文件格式对象的pattern字段进行绝对化处理
     * @param {Function} resolve 接收一个路径处理函数
     * @returns {Function(objectPattern: Pattern): Pattern}
     */
    ...
    Object.keys(preprocessors).forEach(function(pattern) {
        ...
        normalizedPreprocessors[normalizedPattern] =
            helper.isString(preprocessors[pattern]) ?
            [preprocessors[pattern]] : preprocessors[pattern];
    });


    // define custom launchers/preprocessors/reporters
    // - create an inlined plugin
